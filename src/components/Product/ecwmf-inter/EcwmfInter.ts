...
export default class EcwmfInter extends Vue {
  ...
  //获取最新时间
  async getLatestHour() {
    let latestHourUrl = 'http://10.148.16.217:11160/renyin5/satelite/ecmwf/latest'
    let res = await axios({
      adapter: jsonp,
      url: latestHourUrl,
      params: {
    ...
    if (res.data.stateCode === 0) {
      ...
      this.ecwmfDate = moment(latestHour).format('YYYY/MM/DD')
      this.utcSelected = Number(moment(latestHour).format('HH'))
    } else {
      this.ecwmfDate = moment().format('YYYY/MM/DD')
      this.utcSelected = 0
    }
  }
  ...
  changeUrl() {
    ...
    let url = `http://10.148.16.217:11160/renyin5/satelite/img/ecmwf?product=${product}&time=${time}&forecast=${forecast}&area=${area}`
